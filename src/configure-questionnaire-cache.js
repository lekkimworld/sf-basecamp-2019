...
module.exports = pool => {
    ...
    fetch(`${process.env.SF_LOGIN_URL || "https://login.salesforce.com"}/services/oauth2/token`, {
        ...
        const promises = [
            ...
            return pool.query(`select name, sfid, sorting__c sorting, image__c image, answer__c anwserid from salesforce.basecamp_question__c where version__c='${row.versionid}' order by sorting__c asc;`)
        }))
        ...
        return Promise.all([
            Promise.resolve(ctx),
            pool.query(`select sfid answerid, question__c questionid, name as text, sorting__c sorting from salesforce.basecamp_answer__c where question__c in ('${questionIds.join(`','`)}') order by questionid, sorting asc;`)
        ]);
